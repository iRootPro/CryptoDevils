...
const StatBar: FC = () => {
    ...
    const dominance = `BTC: ${formatPercent(data.data.market_cap_percentage.btc / 100)} ETH: ${formatPercent(data.data.market_cap_percentage.eth / 100)}`;
    return (
        <div className={styles.statBar}>
            <div>
                <Text className={`${styles.statType} ${styles.type}`}>Cryptos: <span>{coins}</span></Text>
                <Text className={`${styles.statType} ${styles.type}`}>Exchanges: <span>{exchanges}</span></Text>
                <Text className={`${styles.statType} ${styles.type}`}>Market Cap: <span>{formatUSDNoDecimal(marketCap)}</span></Text>
                <Text className={`${styles.statType} ${styles.type}`}>Volume: <span>{formatUSDNoDecimal(volume)}</span></Text>
                <Text className={`${styles.statType} ${styles.type}`}>Dominance: <span>{dominance}</span></Text>
